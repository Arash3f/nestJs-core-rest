...
/**
 * ? app exception filter for parse project errors
 */
@Catch()
export class CoreExceptionFilter implements ExceptionFilter {
  /**
   * ? import app config service
   * @param config App config service
   */
  constructor(private config: EnvConfigService) {}

  /**
   * ? parse error to final tyle
   * @param exception app exception
   * @param host requst and response object
   */
    ...
    response.status(errorData.statusCode ?? HttpStatus.INTERNAL_SERVER_ERROR).send(errorData)
  }
}
