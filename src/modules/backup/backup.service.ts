...
export class BackupService {
    ...
    startCronJob() {
        const job = new CronJob(
            this.apiConfigService.backupCronPattern,
            async () => {
    ...
    async backupDatabase(): Promise<LinkOutput> {
        ...
        try {
            ...
            if (stderr != "") {
                ...
                throw this.errorService.throwErrorToClient({
                    errorData: BackUpErrors.ThereWasProblemInTakingBackup,
                })
            }
        } catch (e) {
    ...
    private buildDbConfig() {
        return {
            dbCommandPassword: this.prepareDatabasePasswordFormat(
                this.apiConfigService.databaseConfig.password,
            ),
