...
describe("User", () => {
  ...
  it("+ Me API Successfuly", async () => {
     ...
     * * Test Me Api
     */
    const { data: userData } = await api.main.user.me()
  ...
  it("+ CreateUser API Successfuly", async () => {
     ...
     * * Test createUser Api
     */
    const { data: createResponse } = await api.main.user.createUser({
      name,
      username,
      password,
      role,
  ...
  it("- CreateUser API UnSuccessfuly (UserIsNotAuthorized)", async () => {
     ...
     */
    try {
      await api.main.user.createUser({
        name,
        username,
        password,
        role,
  ...
  it("+ ReadUsers API Successfuly (Where)", async () => {
    ...
    const { data: userCount } = await api.main.user.readUsers({})
     ...
     * * Filter name
     */
    const { data: findUser01 } = await api.main.user.readUsers({
      where: {
        name: "UsS00",
      },
    })
     ...
     * * Filter username
     */
    const { data: findUser02 } = await api.main.user.readUsers({
      where: {
        username: "diodfV--78",
      },
    })
     ...
     * * Filter active
     */
    const { data: findUser03 } = await api.main.user.readUsers({
      where: {
        active: false,
      },
    })
     ...
     * * Filter role
     */
    const { data: findUser04 } = await api.main.user.readUsers({
      where: {
        role: Role.Member,
      },
    })
  ...
  it("+ UpdateUser API Successfuly", async () => {
    ...
    const { data: updateUser01 } = await api.main.user.updateUser({
      where: {
        id: user01.id,
      },
      data: {
  ...
  it("+ DeleteUser API Successfuly", async () => {
     ...
     * * Request to server
     */
    await api.main.user.deleteUser({
      id: user01.id,
    })
